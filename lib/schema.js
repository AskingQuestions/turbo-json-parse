...
function convertToRawSchema (jsons, opts) {
  ...
  if (typeof jsons.type === 'object' && Array.isArray(jsons.type)) {
    s.type = MULTI
    s.types = jsons.type.map(
      (t) => convertToRawSchema(typeof t === 'string' ? {type: t} : t, opts)
    )
    return s
  }
